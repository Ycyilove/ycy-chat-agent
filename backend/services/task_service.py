...
class AgentTaskService:
        ...
        if self._orchestrator_provider is not None:
            ...
            async for event in orchestrator.run(
                ...
                if event_type == "thinking":
                    ...
                    logger.info(
                        "[dsh][task_service] approval_required stored: "
                        "id=%s total_pending=%d",
                        approval_id, len(self._pending),
                    )
                    ...
                    tool_call = event["tool_call"]
                    logger.info(
                        "[dsh][task_service] tool_start: %s params=%r",
                    ...
                    result = event["result"]
                    logger.info(
                        "[dsh][task_service] tool_result: %s success=%r error=%r",
    ...
    async def stream_approval(
        ...
        logger.info(
            "[dsh][task_service] stream_approval calling _execute_tool: %s",
        ...
        logger.info(
            "[dsh][task_service] stream_approval _execute_tool finished: %s",
            pending.tool_call.tool_name,
        )